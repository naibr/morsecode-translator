...
public class MorseCode {
    ...
    public String convertToMorseCode(String inputWord) {

        String convertedWord = "";
        for (int i = 0; i < inputWord.length(); i++) {
            Character character = Character.toUpperCase(inputWord.charAt(i));
            convertedWord += CodeTranslator.get(character.toString());
        }
    ...
    public String convertToEnglish(String morseCode) {
        ...
        for (int i = 0; i < characters.length; i++) {
            englishWord += CodeTranslator.inverse().get(characters[i] + " ");
        }
